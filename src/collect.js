  ...
  */
        ...
        if (!anyMatch) {
          throw new Error(
            utils.createHelp(
              schema,
              globals,
              'Required one of: ' + utils.formatRequiredList(anyRequired)
            )
          );
        }
      });
    ...
    while (argv.length) {
      ...
      if (isPositional) {
        ...
        if (matchingCommand) {
          ...
          if (!matchingArg) {
            throw new Error(
              utils.createHelp(schema, globals, 'Unknown argument: ' + arg)
            );
            // Known command
  ...
  function collect(rootNode, argv) {
    ...
    if (!Array.isArray(argv)) {
      throw new Error(
        'argv must be an array of strings, but got ' + typeof argv
      );
    }
